...
class FactorArguments(Arguments):
    ...
    amp_dtype: Optional[torch.dtype] = field(
        default=None,
        metadata={
            "help": "Data type for automatic mixed precision (AMP). If `None`, AMP is disabled."
        },
    )
    amp_scale: float = field(
        default=2.0**16,
        metadata={
            "help": "Scale factor for AMP (only applicable when `amp_dtype=torch.float16`)."
        },
    )
    has_shared_parameters: bool = field(
        default=False,
        metadata={
            "help": "Indicates whether shared parameters are present in the model's forward pass."
        },
    )
    ...
    covariance_max_examples: Optional[int] = field(
        default=100_000,
        metadata={
            "help": "Maximum number of examples for fitting covariance matrices. Uses entire dataset if `None`."
        },
    )
    covariance_data_partitions: int = field(
        default=1,
        metadata={
            "help": "Number of partitions to divide the dataset into for covariance matrix computation."
        },
    )
    ...
    lambda_max_examples: Optional[int] = field(
        default=100_000,
        metadata={
            "help": "Maximum number of examples for fitting Lambda matrices. Uses entire dataset if `None`."
        },
    )
    lambda_data_partitions: int = field(
        default=1,
        metadata={
            "help": "Number of partitions to divide the dataset into for Lambda matrix computation."
        },
    )
    ...
    offload_activations_to_cpu: bool = field(
        default=False,
        metadata={
            "help": "If `True`, offloads cached activations to CPU memory when computing per-sample gradients."
        },
    )
    ...
    def __post_init__(self) -> None:
        if (
            self.covariance_max_examples is not None
            and self.covariance_max_examples <= 0
        ):
...
class ScoreArguments(Arguments):
    ...
    data_partitions: int = field(
        default=1,
        metadata={
            "help": "Number of partitions to divide the dataset for influence score computation."
        },
    )
    ...
    compute_per_module_scores: bool = field(
        default=False,
        metadata={
            "help": "If `True`, computes separate scores for each module instead of summing across all."
        },
    )
    ...
    query_gradient_accumulation_steps: int = field(
        default=1,
        metadata={
            "help": "Number of query batches to accumulate before processing training examples."
        },
    )
    ...
    use_measurement_for_self_influence: bool = field(
        default=False,
        metadata={
            "help": "If `True`, uses the measurement (instead of the loss) for computing self-influence scores."
        },
    )
    ...
    query_gradient_svd_dtype: torch.dtype = field(
        default=torch.float32,
        metadata={
            "help": "Data type for singular value decomposition (SVD) of query gradient."
        },
    )
    per_sample_gradient_dtype: torch.dtype = field(
        default=torch.float32,
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            partition <= 0
            for partition in [self.data_partitions, self.module_partitions]
        ):
        ...
        if (
            self.query_gradient_low_rank is not None
            and self.query_gradient_low_rank <= 0
        ):
