...
class FactorConfig(metaclass=ABCMeta):
    ...
    def prepare(
        self, storage: STORAGE_TYPE, score_args: Any, device: torch.device
    ) -> None:
    ...
    def precondition_gradient(
        ...
        """
        raise NotImplementedError(
            "Subclasses must implement the `precondition_gradient` method."
        )
...
class Diagonal(FactorConfig, factor_strategy=FactorStrategy.DIAGONAL):
    ...
    def prepare(
        ...
        lambda_matrix = storage[LAMBDA_MATRIX_NAME].to(
            dtype=LAMBDA_DTYPE, device=device
        )
        ...
        storage[LAMBDA_MATRIX_NAME] = lambda_matrix.to(
            dtype=score_args.precondition_dtype, device="cpu"
        ).contiguous()
...
class Kfac(FactorConfig, factor_strategy=FactorStrategy.KFAC):
    ...
    def prepare(
        ...
        storage[ACTIVATION_EIGENVECTORS_NAME] = (
            storage[ACTIVATION_EIGENVECTORS_NAME]
            .to(dtype=score_args.precondition_dtype)
            .contiguous()
        )
    ...
    def precondition_gradient(
        ...
        gradient = torch.matmul(
            gradient_eigenvectors.t(), torch.matmul(gradient, activation_eigenvectors)
        )
        gradient.mul_(lambda_matrix)
        gradient = torch.matmul(
            gradient_eigenvectors, torch.matmul(gradient, activation_eigenvectors.t())
        )
        return gradient
