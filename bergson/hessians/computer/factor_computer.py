...
class FactorComputer(Computer):
    ...
    def _configure_and_save_factor_args(
        self,
        factor_args: Optional[FactorArguments],
        factors_output_dir: Path,
    ...
    def fit_lambda_matrices(
        ...
        overwrite_output_dir: bool = False,
        load_from_factors_name: Optional[str] = None,
        load_factors_output_dir: Optional[str] = None,  # Louis edit
    ) -> None:
        ...
        if load_factors_output_dir is None:
            if load_from_factors_name is not None:
                self.logger.info(
            ...
            self.logger.info(
                f"Will be loading eigendecomposition results from factors with name `{load_factors_output_dir}`."
            )
            load_factors_output_dir = Path(load_factors_output_dir)
